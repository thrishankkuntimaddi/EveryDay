...
<title>Verify your EveryDay account</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .card{background:#13131f;border:1px solid rgba(124,58,237,0.25);border-radius:16px;overflow:hidden}
  .top-bar{height:3px;background:linear-gradient(90deg,#7c3aed,#06b6d4)}
  .body{padding:36px 32px}
  .logo{display:flex;align-items:center;gap:10px;margin-bottom:28px}
  .logo-icon{width:36px;height:36px;background:linear-gradient(135deg,#7c3aed,#06b6d4);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:18px}
  ...
  .btn{display:block;background:linear-gradient(135deg,#7c3aed,#9d64f8);color:#fff;text-decoration:none;text-align:center;font-size:15px;font-weight:700;padding:14px 24px;border-radius:10px;letter-spacing:0.01em;margin-bottom:24px}
...
<div class="wrap">
  <div class="card">
    ...
    <div class="body">
      <div class="logo">
        ...
        <span class="logo-name">EveryDay</span>
      </div>

      <h1>Verify your email</h1>
      <p class="sub">
        You're one step away from building identity through consistency.<br><br>
  ...
  <p class="footer">
    If you didn't create an EveryDay account, you can safely ignore this email.<br>
...
export async function sendVerificationEmail({ to, uid, token }) {
  ...
  const info = await transporter.sendMail({
    from:    `"EveryDay" <${process.env.EMAIL_FROM}>`,
    to,
    subject: '⚡ Verify your EveryDay account',
    html:    _verificationHTML(verifyUrl, to),
    text:    `Verify your EveryDay account:\n\n${verifyUrl}\n\nThis link expires in 24 hours.`,
  });
